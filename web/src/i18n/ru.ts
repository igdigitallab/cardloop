...
export const ru = {
  ...
  'chat.ask_hint': 'Агент спрашивает перед каждым действием, которое что-то меняет: любая Bash-команда, правка или запрос в сеть вешает здесь карточку Разрешить / Всегда разрешать / Запретить и шлёт пуш на телефон. Чтение и поиск идут свободно. По умолчанию выключено; запоминается на чат.',
