...
export const ru = {
  ...
  // ── Agents tab (spec-091 Phase 1; FX5 объединил сюда отдельную вкладку CLAUDE.md) ──
  'agents.loading': 'Загрузка ролей агентов…',
  'agents.unavailable': 'Роли агентов пока недоступны в этой сборке.',
  'agents.retry': 'Повторить',
  'agents.main_title': 'Главный агент — CLAUDE.md',
  'agents.main_hint': 'Это собственный CLAUDE.md проекта — файл инструкций главного агента. Редактируется прямо здесь, изменения действуют со следующего хода.',
  ...
  // FX5, решение 2: любая роль теперь открывается в реальном редакторе (режима "только
  // просмотр" больше нет); эта подсказка видна только при редактировании ВСТРОЕННОЙ роли.
  'agents.builtin_note': 'Встроенные роли поставляются с Cardloop и доступны только для чтения — сохранение здесь создаёт или обновляет копию в области проекта; сам встроенный файл не меняется.',
  'agents.save_target_label': 'Сохранится в:',
