...
export function BoardTab({ projectId, isActive = true, onDiscuss }: Props) {
  ...
  function renderCard(
    ...
    return (
      <div
        ...
        // 9f16df: right-click anywhere on the card opens the same action menu at the cursor.
        onContextMenu={(e) => {
          e.preventDefault()
          const x = e.clientX, y = e.clientY
          const rect = { left: x, top: y, right: x, bottom: y, width: 0, height: 0, x, y, toJSON() { return {} } } as DOMRect
          setCardMenu({ rect, sections: buildCardSections(card, { columnKey, parkIdx, canShowResult }) })
        }}
      >
        {parkIdx >= 0 && (
          <input
        ...
        />
        {/* 9f16df: desktop-only quick-action row, revealed on card hover (mobile keeps the
            kebab/long-press menu unchanged). Mirrors the primary menu section. */}
        <div className="board-card-hover-actions">
          {buildCardSections(card, { columnKey, parkIdx, canShowResult })[0]?.items.map((it, i) => (
            <button
              key={i}
              className="board-card-hover-btn"
  ...
  function buildCardSections(
    ...
    if (onDiscuss) {
      primary.push({ label: 'Discuss', icon: '💬', onClick: () => onDiscuss({ cardId: card.id, title: card.text }) })
    }
