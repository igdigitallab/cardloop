  ...
  async function handleAddTask() {
    ...
    try {
      ...
      const instruction = [
        ...
        ``,
        `You are running in the cwd of THIS project (check: pwd). The config lives in data/topics.json at the root of the Cardloop server — NOT inside the current project. Find it with: find $HOME -maxdepth 4 -path "*/data/topics.json" 2>/dev/null | head -1`,
        ``,
        `STEPS:`,
        `1. Open data/topics.json at the Cardloop server root (path found above). Find the block whose "cwd" matches your pwd. Make changes ONLY in that block. Save valid JSON (don't break other blocks).`,
        ``,
        ...
        ``,
        `5. Restart is NOT needed: the cockpit re-reads topics.json from disk on the fly (hot-reload). After saving, the Logs tab will work immediately. Do NOT restart the Cardloop server and do not write "restart needed".`,
        ``,
