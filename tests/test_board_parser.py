...
## Backlog
- [ ] First task <!--ops:aabbcc-->
- [ ] Second task <!--ops:112233-->

## In Progress
- [~] Task in progress <!--ops:deadbe-->

## Review
- [?] Under review <!--ops:cafe01-->
...
## In Progress
- [ ] Agent already picked this up <!--ops:111111-->
"""
...
Preamble: some free text before the first column.

## Backlog
- [ ] Task with preamble <!--ops:aabbcc-->
"""
...
def test_parse_canonical():
    ...
    assert ip_card["text"] == "Task in progress"
...
## In Progress
- Task without checkbox in In Progress
"""
    _, cols = _parse_tasks(text)
    assert len(cols["in_progress"]) == 1
    assert cols["in_progress"][0]["text"] == "Task without checkbox in In Progress"
...
def test_parse_preamble_preserved():
    ...
    assert "free text" in preamble, "Free text must be in the preamble"

    serialized = _serialize_tasks(preamble, cols, "some-project")
    assert "free text" in serialized, "Preamble text must survive serialization"
...
def test_parse_preamble_not_in_backlog():
    ...
    assert not any("Description" in t for t in all_texts), "Description from preamble must not become a card"
    assert not any("free text" in t for t in all_texts)
...
## Backlog
- [ ] Real card <!--ops:aabbcc-->
- Item 1 from some list
- Item 2 from the same list
"""
...
def test_count_potential_cards_empty():
    ...
    assert _count_potential_cards("# heading\n\nJust text") == 0
...
def test_serialize_unknown_section_dropped():
    ...
    text = """\
## Backlog
- [ ] Normal card <!--ops:aabbcc-->

## Notes
- This is a note, not a card

## In Progress
- [~] Task in progress <!--ops:112233-->
"""
    _, cols = _parse_tasks(text)
    ...
    assert "This is a note" not in serialized
...
def test_parse_card_with_description():
    ...
    text = """\
## Backlog
- [ ] Short title <!--ops:abc111-->
  > First description line.
  > Second description line.
- [ ] Card without description <!--ops:abc222-->
"""
    _, cols = _parse_tasks(text)
    assert len(cols["backlog"]) == 2
    ...
    assert card_with_desc["text"] == "Short title"
    assert card_with_desc["description"] == "First description line.\nSecond description line."
...
def test_serialize_round_trip_with_description():
    ...
    original = """\
## Backlog
- [ ] Title of the card <!--ops:xyz789-->
  > Description first line.
  > Description second line.
- [ ] Without description <!--ops:nnn000-->
"""
    ...
    assert "  > Description first line." in serialized
    assert "  > Description second line." in serialized
    ...
    assert card_with["text"] == "Title of the card"
    assert card_with["description"] == "Description first line.\nDescription second line."
...
"""
    _, cols = _parse_tasks(text)
    assert len(cols["backlog"]) == 2
    assert cols["backlog"][0]["description"] == "Description of the first."
    assert cols["backlog"][1]["description"] == "Description of the second."
...
## In Progress
- [ ] Another task <!--ops:bbb002-->
"""
    _, cols = _parse_tasks(text)
    assert cols["backlog"][0]["description"] == "Description."
