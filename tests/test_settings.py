...
def test_global_settings_hot_reload(settings_tmp):
    ...
    _webapp._SETTINGS_MTIME = 0.0  # force re-read (mtime may match within the same tick)
...
def test_global_settings_broken_file_keeps_cache(settings_tmp):
    ...
    assert _webapp._get_global_setting("scan_interval_sec", 300) == 60  # populate cache
    (settings_tmp / "settings.json").write_text("{not json", encoding="utf-8")
    _webapp._SETTINGS_MTIME = 0.0
    assert _webapp._get_global_setting("scan_interval_sec", 300) == 60  # broken file → previous cache
...
async def test_card_run_mode_git_disabled_is_legacy():
    # git_enabled=False → legacy mode, no git access
