"""
Tests for webapp._format_tool — the unified tool-event formatter.

Used in three places: chat SSE, bus publish, and session-history.
Regression = breaks live chat, history parsing, and the activity bus simultaneously.
"""
...
def test_multiedit_non_list_edits():
    """If edits is not a list (malformed SDK input) — count=0, no crash."""
...
def test_write_non_string_content():
    """content is not a string — preview is empty, no crash."""
...
def test_unknown_tool_non_string_first():
    """First argument is not a string — str() it (number, dict)."""
...
def test_non_dict_input_treated_as_empty():
    """If inp is not a dict (malformed SDK input) — treat as {}, no crash."""
...
def test_all_tools_return_name_and_kind():
    """Contract: every variant returns at least 'name' and 'kind' — the UI depends on them."""
    for tool_name in ("Bash", "Edit", "Write", "Read", "Glob", "Grep", "MultiEdit",
                      "NotebookEdit", "RandomThing"):
        out = _format_tool(tool_name, {})
        assert "name" in out, f"{tool_name}: missing 'name'"
        assert "kind" in out, f"{tool_name}: missing 'kind'"
