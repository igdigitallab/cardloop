...
def test_skip_restart_sigterm_teardown_noise():
    """spec-052 Phase 1a: the SDK message-reader dying on a signal during a
    self-restart / aborted turn (exit code -15 / -9 / 143) is a teardown
    artifact, not a product bug — it must not become an incident card."""
    log = (
    ...
    assert _parse_log_errors("ERROR: worker Command failed with exit code -9") == []
