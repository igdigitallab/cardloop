"""
Тесты сканера инцидентов: парсеры логов/pytest + дедуп + ingest в доску.

CRITICAL: регрессия = либо потеря ошибок (не попали в Failed),
либо лавина дубликатов (один traceback → 1000 карточек за ночь).
"""
import pytest
...
def test_dedup_identical_traceback_in_same_log():
    """Один и тот же traceback дважды в логе → одна запись (дедуп по hash внутри прогона)."""
    log = PYTHON_TRACEBACK + "\n" + PYTHON_TRACEBACK
    errors = _parse_log_errors(log)
    assert len(errors) == 1, "Дубль traceback должен быть схлопнут в одну запись"
...
def test_skip_deprecation_warnings():
    """DeprecationWarning не должна попадать в инциденты — это шум."""
    log = "ERROR: DeprecationWarning: foo() is deprecated, use bar()"
    errors = _parse_log_errors(log)
    assert len(errors) == 0, "Deprecation-предупреждения должны быть отфильтрованы"
...
"""
    errors = _parse_log_errors(log)
    # Одна запись от python-traceback парсера, ERROR-строка с "Traceback" должна быть пропущена
    assert len(errors) == 1
    assert errors[0]["type"] == "ZeroDivisionError"
...
def test_pytest_dedup_same_failure():
    """Один и тот же FAILED дважды → одна запись."""
    out = PYTEST_OUTPUT + "\n" + PYTEST_OUTPUT
    fails = _parse_pytest_failures(out)
    assert len(fails) == 2  # не 4


def test_pytest_failure_with_no_reason():
    """FAILED без reason — всё равно должен попасть в список."""
...
def test_pytest_parametrized_test_name():
    """Параметризованные тесты test_x[case-1] — name содержит брэкеты."""
...
def test_norm_msg_strips_numbers():
    """Числа (PID/timestamp) должны быть нормализованы → дубли с разными числами имеют один hash."""
    h1 = _hash6(_norm_msg("connection lost at 1234567890"))
    h2 = _hash6(_norm_msg("connection lost at 9876543210"))
    assert h1 == h2, "Хеши должны совпасть после нормализации чисел"


def test_norm_msg_strips_paths():
    """Пути в сообщении нормализуются — один и тот же баг в разных файлах сворачивается."""
...
def test_norm_msg_preserves_exception_type_difference():
    """Разные exception-типы дают разные хеши."""
...
def test_format_and_parse_desc_round_trip():
    """format → parse возвращает исходные ключи."""
    meta = {
        "source": "log",
        "seen": "3",
...
def test_format_desc_keeps_excerpt_compact():
    """Многострочный excerpt → одна строка с разделителем (description в формате '  > line')."""
    meta = {"source": "log", "seen": "1", "excerpt": "line1\nline2\nline3"}
    desc = _format_incident_desc(meta)
    # Excerpt должен стать одной строкой в файле
...
def test_parse_desc_ignores_unknown_lines():
    """Ненужные строки в description (например, агент дописал что-то) игнорируются."""
...
def test_incident_title_truncates_long_message():
    ...
    title = _incident_title(err)
    assert len(title) <= 120, f"Заголовок слишком длинный: {len(title)}"
