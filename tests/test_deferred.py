...
def deferred_app(fake_ctx):
    ...
    app.router.add_post("/api/deferred/{id}/confirm", _webapp.api_deferred_confirm)
...
# ─────────────────────────── rate-limit window renewal regression ────────────


@pytest.mark.asyncio
async def test_strict_reset_fires_after_window_renews(fake_ctx):
    """Regression: after the 5h window resets, the API returns a NEW resets_at ~5h in the future.
    Without _target_resets_at pinning the loop would compare against that new value and never fire.
    With the fix the first iteration stores the original target; subsequent iterations use it."""
    fake_ctx["topics"]["100:10"] = _make_topic("myproject")

    now = time.time()
    # Simulate: rate-limited with resets_at 100s in the past (window just reset)
    original_resets_at = now - 100

    _webapp._save_deferred([{
        "id": "def-window-renewal",
        "project": "myproject",
        "session_key": "100:10",
        ...
        "attempts": 0,
        "_jitter": 30,
        # Pre-pin the target as if the loop already stored it during the rate-limit window
        "_target_resets_at": original_resets_at,
    }])

    # API now returns NEW resets_at = now + 18000 (fresh 5h window after reset)
    # Without the fix this would prevent firing; with it we ignore the live value.
    new_window_usage = {
        "five_hour": {
            "utilization": 0.05,
            "resets_at": now + 18000,
    ...
    with patch.object(_webapp, "_get_cached_usage_data", new_callable=AsyncMock, return_value=new_window_usage), \
         patch.object(_webapp, "_spawn_bg", side_effect=mock_spawn_bg), \
         patch.object(_webapp, "_notify_operator", new_callable=AsyncMock):
        with patch("asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
            call_count = 0
    ...
    records = _webapp._load_deferred()
    fired = next(r for r in records if r["id"] == "def-window-renewal")
    assert fired["status"] == "fired", (
        f"Expected 'fired' but got '{fired['status']}' — "
        "strict record must fire at the pinned target, not the live resets_at"
    )
    assert len(spawned) == 1


@pytest.mark.asyncio
async def test_strict_reset_pins_target_resets_at_on_first_iteration(fake_ctx):
    """On the first loop iteration the record has no _target_resets_at yet.
    The loop must store the live resets_at as the target and NOT fire immediately."""
    fake_ctx["topics"]["100:10"] = _make_topic("myproject")

    now = time.time()
    future_resets = now + 300  # reset boundary 5 minutes away

    _webapp._save_deferred([{
        "id": "def-pin-target",
        "project": "myproject",
        "session_key": "100:10",
        "prompt": "send after reset",
        "fire_at": None,
        "fire_on_reset": True,
        "strict_reset": True,
        "status": "pending",
        ...
        "error": None,
        "attempts": 0,
        # No _target_resets_at yet — loop must store it
    }])

    usage = {"five_hour": {"utilization": 0.95, "resets_at": future_resets, "status": "allowed"}}
    ...
    with patch.object(_webapp, "_get_cached_usage_data", new_callable=AsyncMock, return_value=usage), \
         patch.object(_webapp, "_spawn_bg", side_effect=mock_spawn_bg), \
         patch.object(_webapp, "_notify_operator", new_callable=AsyncMock):
        with patch("asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
            call_count = 0
    ...
    # Must NOT have fired (reset boundary is in the future)
    assert len(spawned) == 0, "Record must not fire before its target resets_at"
    records = _webapp._load_deferred()
    rec = next(r for r in records if r["id"] == "def-pin-target")
    assert rec["status"] == "pending"
    # _target_resets_at must have been stored on this first iteration
    assert rec.get("_target_resets_at") == future_resets, (
        f"Expected _target_resets_at={future_resets!r}, got {rec.get('_target_resets_at')!r}"
    )
...
@pytest.mark.asyncio
async def test_auto_resume_env_off_defaults_to_ask(tmp_path):
    """spec-051: with env=0 and no per-project policy, default mode is 'ask' →
    an awaiting_confirmation record is created and a prompt is published (no longer silent)."""
    ctx = _make_fake_ctx_with_topic(tmp_path)
    result_event = {"type": "result", "api_error_status": 429}

    with patch.object(_webapp, "_AUTO_RESUME_ON_RATE_LIMIT", 0), \
         patch.object(_webapp, "_bus_publish") as mock_pub, \
         patch.object(_webapp, "_notify_operator", new_callable=AsyncMock):
        await _webapp._maybe_auto_resume(
            ctx=ctx,
            session_key="100:10",
    ...
    records = _webapp._load_deferred()
    assert len(records) == 1
    assert records[0]["status"] == "awaiting_confirmation"
    # prompt event published to the project chat
    kinds = [c.args[1].get("kind") for c in mock_pub.call_args_list if len(c.args) > 1]
    assert "rate_limit_prompt" in kinds


# ─────────────────────────── spec-051: per-project resume policy ───────────────────────────


def test_resume_mode_for_explicit_and_fallback():
    """_resume_mode_for: explicit value wins; otherwise env fallback (1→always, 0→ask)."""
    assert _webapp._resume_mode_for({"auto_resume_mode": "never"}) == "never"
    assert _webapp._resume_mode_for({"auto_resume_mode": "ALWAYS"}) == "always"
    with patch.object(_webapp, "_AUTO_RESUME_ON_RATE_LIMIT", 1):
        assert _webapp._resume_mode_for({}) == "always"
    with patch.object(_webapp, "_AUTO_RESUME_ON_RATE_LIMIT", 0):
        assert _webapp._resume_mode_for({}) == "ask"


@pytest.mark.asyncio
async def test_auto_resume_mode_never_no_record(tmp_path):
    """spec-051: project policy 'never' → no record even on a 429."""
    ctx = _make_fake_ctx_with_topic(tmp_path)
    ctx["topics"]["100:10"]["auto_resume_mode"] = "never"
    result_event = {"type": "result", "api_error_status": 429}

    with patch.object(_webapp, "_notify_operator", new_callable=AsyncMock) as mock_notify, \
         patch.object(_webapp, "_bus_publish") as mock_pub:
        await _webapp._maybe_auto_resume(
            ctx=ctx, session_key="100:10",
            original_prompt="Run a task", last_result_event=result_event,
        )

    assert _webapp._load_deferred() == []
    mock_notify.assert_not_called()
    mock_pub.assert_not_called()


@pytest.mark.asyncio
async def test_auto_resume_mode_always_creates_pending(tmp_path):
    """spec-051: project policy 'always' (env off) → pending record, no prompt."""
    ctx = _make_fake_ctx_with_topic(tmp_path)
    ctx["topics"]["100:10"]["auto_resume_mode"] = "always"
    result_event = {"type": "result", "api_error_status": 429}
    ...
    assert len(records) == 1
    assert records[0]["status"] == "pending"
    prompt_kinds = [c.args[1].get("kind") for c in mock_pub.call_args_list if len(c.args) > 1]
    assert "rate_limit_prompt" not in prompt_kinds


def _seed_awaiting(session_key="100:10", did="def-conf1"):
    rec = {
        "id": did, "project": "myproject", "session_key": session_key,
        "prompt": "Continue the interrupted task", "fire_at": None,
        "fire_on_reset": True, "created": _webapp._utcnow_iso(),
        "status": "awaiting_confirmation", "fired_at": None, "error": None,
        "attempts": 0, "auto_resume": True, "auto_resume_count": 1,
    }
    _webapp._save_deferred([rec])
    return did


@pytest.mark.asyncio
async def test_confirm_yes_makes_pending(aiohttp_client, deferred_app, fake_ctx):
    did = _seed_awaiting()
    client = await aiohttp_client(deferred_app)
    resp = await client.post(f"/api/deferred/{did}/confirm",
                             json={"decision": "yes"}, headers=auth_headers(fake_ctx))
    assert resp.status == 200
    body = await resp.json()
    assert body["status"] == "pending"
    assert _webapp._load_deferred()[0]["status"] == "pending"


@pytest.mark.asyncio
async def test_confirm_no_cancels(aiohttp_client, deferred_app, fake_ctx):
    did = _seed_awaiting()
    client = await aiohttp_client(deferred_app)
    resp = await client.post(f"/api/deferred/{did}/confirm",
                             json={"decision": "no"}, headers=auth_headers(fake_ctx))
    assert resp.status == 200
    assert _webapp._load_deferred()[0]["status"] == "cancelled"


@pytest.mark.asyncio
async def test_confirm_remember_writes_policy(aiohttp_client, deferred_app, fake_ctx):
    did = _seed_awaiting()
    fake_ctx["topics"]["100:10"] = {"project": "myproject", "cwd": "/tmp/p", "model": "sonnet"}
    saved = []
    fake_ctx["save_topics"] = lambda: saved.append(True)
    client = await aiohttp_client(deferred_app)
    resp = await client.post(f"/api/deferred/{did}/confirm",
                             json={"decision": "yes", "remember": True}, headers=auth_headers(fake_ctx))
    assert resp.status == 200
    assert (await resp.json())["remembered"] == "always"
    assert fake_ctx["topics"]["100:10"]["auto_resume_mode"] == "always"
    assert saved  # persisted


@pytest.mark.asyncio
async def test_confirm_idempotent_on_resolved(aiohttp_client, deferred_app, fake_ctx):
    did = _seed_awaiting()
    _webapp._save_deferred([{**_webapp._load_deferred()[0], "status": "fired"}])
    client = await aiohttp_client(deferred_app)
    resp = await client.post(f"/api/deferred/{did}/confirm",
                             json={"decision": "yes"}, headers=auth_headers(fake_ctx))
    assert resp.status == 200
...
@pytest.mark.asyncio
async def test_confirm_bad_decision_rejected(aiohttp_client, deferred_app, fake_ctx):
    did = _seed_awaiting()
    client = await aiohttp_client(deferred_app)
    resp = await client.post(f"/api/deferred/{did}/confirm",
                             json={"decision": "maybe"}, headers=auth_headers(fake_ctx))
    assert resp.status == 400
