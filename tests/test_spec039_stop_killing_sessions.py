...
# ─────────────────────────── Test 1: /reset evicts live client ──────────────────────────────────

@pytest.mark.asyncio
async def test_cmd_reset_evicts_live_client():
    """/reset (cmd_reset) must evict the live client from the module-level registry.

    With PERSISTENT_CLIENT=1 a previous turn populates _live_clients.  cmd_reset must
    call _evict_live_client so the next turn starts with a truly fresh subprocess.
    """
    session_key = "111:7"

    # Pre-populate the module-level registry directly.
    entry = _make_live_entry(session_key)
    bot._live_clients[session_key] = entry

    try:
        # Minimal fake PTB update/context
        fake_update = MagicMock()
        fake_update.effective_chat.id = 111
        fake_update.effective_message.message_thread_id = 7
        fake_update.effective_user.id = 42
        fake_update.effective_message.text = "/reset"

        fake_context = MagicMock()
        fake_context.args = []

        # Stub out network calls and authorization
        with patch.object(bot, "authorized", return_value=True), \
             patch.object(bot, "key_of", return_value=session_key), \
             patch.object(bot, "send", new=AsyncMock()), \
             patch.object(bot, "sessions", {session_key: "sess-old"}), \
             patch.object(bot, "save_sessions", MagicMock()), \
             patch.object(bot, "context_warned", set()), \
             patch.object(bot, "_tg_queue_clear", return_value=0), \
             patch.object(bot, "topics", {}), \
             patch.object(bot, "binding_for", return_value={"project": "test-proj"}):

            await bot.cmd_reset(fake_update, fake_context)

        # The live client must have been evicted and disconnected.
        assert session_key not in bot._live_clients, (
            "cmd_reset must evict the live client from the module-level registry"
        )
        entry.client.disconnect.assert_called_once()
    finally:
        # Cleanup — ensure the entry is gone even if the test fails mid-way
        bot._live_clients.pop(session_key, None)


# ─────────────────────────── Test 2: api_project_rotate does real reset+evict ──────────────────
