...
async def _capture_effort_for_think_mode(aiohttp_client, tmp_path, project_dir, think_mode, sid):
    """Helper: POST a chat with the given think_mode and return the effort run_engine received."""
...
async def test_chat_think_mode_ladder_passthrough(aiohttp_client, tmp_path, project_dir, level):
    """Each ladder value passes through to run_engine 1:1 as the effort override."""
    effort = await _capture_effort_for_think_mode(
        aiohttp_client, tmp_path, project_dir, level, f"s-{level}"
    )
    assert effort == level, f"Expected effort={level!r}, got: {effort!r}"


@pytest.mark.parametrize("bad", ["default", "min", "ultra", "", "HIGH", None])
async def test_chat_think_mode_unknown_passes_none_effort(
    aiohttp_client, tmp_path, project_dir, bad
):
    """An unknown/absent think_mode → effort=None so the engine uses its _DEFAULT_EFFORT."""
    effort = await _capture_effort_for_think_mode(
        aiohttp_client, tmp_path, project_dir, bad, "s-none"
    )
    assert effort is None, f"Expected effort=None for {bad!r}, got: {effort!r}"
