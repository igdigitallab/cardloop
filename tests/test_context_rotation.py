...
async def test_rotate_endpoint_stub_response(aiohttp_client, tmp_path, project_dir):
    ...
    bus_events: list = []
    with patch.object(_webapp, "_bus_publish", side_effect=lambda sk, ev, **kw: bus_events.append(ev)):
        app = _make_app(ctx)
        client = await aiohttp_client(app)
        resp = await client.post(
            "/api/projects/myproject/rotate",
    ...
    # A durable rotation record must reach the timeline (Activity feed), tagged manual.
    rot = [e for e in bus_events if e.get("kind") == "session_rotated"]
    assert rot, f"a session_rotated event must be emitted, got: {[e.get('kind') for e in bus_events]}"
    assert rot[0]["trigger"] == "manual"
...
# ─────────────── Part 5: Cost ledger + revived auto-rotation (cost-tail control) ──────────

def test_append_usage_ledger_writes_row(tmp_path, monkeypatch):
    """engine.append_usage_ledger appends one JSON-parseable row per call."""
    ledger = tmp_path / "usage_ledger.jsonl"
    monkeypatch.setattr(_engine, "USAGE_LEDGER_F", ledger)
    _engine.append_usage_ledger({"ts": 1.0, "entrypoint": "chat", "context_tokens": 138000,
                                 "fresh_tokens": 1500, "cache_read_tokens": 136500, "cost_usd": 0.02})
    _engine.append_usage_ledger({"ts": 2.0, "entrypoint": "card", "context_tokens": 40000,
                                 "fresh_tokens": 500, "cache_read_tokens": 39500, "cost_usd": 0.01})
    lines = ledger.read_text(encoding="utf-8").strip().splitlines()
    assert len(lines) == 2
    r0 = json.loads(lines[0])
    assert r0["entrypoint"] == "chat" and r0["context_tokens"] == 138000


def test_append_usage_ledger_never_raises(tmp_path, monkeypatch):
    """A bad path must NOT raise into a turn — the ledger is strictly best-effort."""
    monkeypatch.setattr(_engine, "USAGE_LEDGER_F", tmp_path / "nope" / "deep" / "ledger.jsonl")
    # parent dir does not exist → open() would raise, but the helper must swallow it
    _engine.append_usage_ledger({"ts": 1.0})  # must not raise


async def test_usage_ledger_endpoint_aggregates(aiohttp_client, tmp_path):
    from aiohttp import web
    ctx = _make_ctx(tmp_path, tmp_path / "p")
    (tmp_path / "p").mkdir(exist_ok=True)
    ledger = ctx["DATA"] / "usage_ledger.jsonl"
    rows = [
        {"ts": time_now(), "entrypoint": "chat", "session_key": "s1", "project": "p",
         "context_tokens": 138000, "fresh_tokens": 1500, "cache_read_tokens": 136500,
         "cost_usd": 0.02, "ultracode": False},
        {"ts": time_now(), "entrypoint": "chat", "session_key": "s1", "project": "p",
         "context_tokens": 300000, "fresh_tokens": 2000, "cache_read_tokens": 298000,
         "cost_usd": 0.05, "ultracode": True},
        {"ts": time_now(), "entrypoint": "card", "session_key": "s2", "project": "p",
         "context_tokens": 40000, "fresh_tokens": 500, "cache_read_tokens": 39500,
         "cost_usd": 0.01, "ultracode": False},
    ]
    ledger.write_text("\n".join(json.dumps(r) for r in rows) + "\n", encoding="utf-8")

    app = web.Application(middlewares=[_webapp.auth_middleware])
    app["ctx"] = ctx
    app.router.add_get("/api/usage/ledger", _webapp.api_usage_ledger)
    client = await aiohttp_client(app)
    resp = await client.get("/api/usage/ledger?days=30", headers=_auth_headers(ctx))
    assert resp.status == 200
    data = await resp.json()
    assert data["total"]["turns"] == 3
    assert data["by_entrypoint"]["chat"]["turns"] == 2
    assert data["by_entrypoint"]["card"]["turns"] == 1
    assert data["ultracode"]["turns"] == 1
    # Heaviest session by peak context surfaces first (the "tail" detector).
    assert data["top_sessions"][0]["session_key"] == "s1"
    assert data["top_sessions"][0]["max_context_tokens"] == 300000


def time_now():
    # tiny indirection so the test never calls a banned Date.now equivalent in scripts;
    # in pytest plain time.time() is fine.
    import time as _t
    return _t.time()
...
async def test_result_frame_carries_cost_thresholds(aiohttp_client, tmp_path, project_dir):
    ...
    with patch.object(_webapp, "CONTEXT_WARN_AT", 200000), \
        ...
        resp = await client.post("/api/projects/myproject/chat",
                                 json={"prompt": "go"}, headers=_auth_headers(ctx))
        assert resp.status == 200
        events = await _read_sse(resp)
...
async def test_auto_rotate_fires_above_cap(aiohttp_client, tmp_path, project_dir):
    """Context >= cap + CONTEXT_ROTATION on + chat OPTED IN (auto_rotate) → session cleared, handoff, bus event."""
    async def fake_engine(**kwargs):
        yield {"type": "text", "text": "hello"}
        yield {"type": "result", "session_id": "sess-big", "context_tokens": 300000}
    ...
    assert SESSION_KEY not in ctx["sessions"], "session must be cleared by auto-rotate"
    assert ctx["pending_handoff"].get(SESSION_KEY) == "prior summary", "handoff must be stored"
    assert any(e.get("kind") == "auto_rotated" for e in bus_events), (
        f"an auto_rotated bus event must be published, got: {[e.get('kind') for e in bus_events]}"
    )
    # A durable session_rotated record must also reach the timeline, tagged auto + the context size.
    rotated = [e for e in bus_events if e.get("kind") == "session_rotated"]
    assert rotated, "a durable session_rotated event must be emitted for the Activity feed"
    assert rotated[0]["trigger"] == "auto"
...
async def test_auto_rotate_skips_below_cap(aiohttp_client, tmp_path, project_dir):
    """Context below CONTEXT_ROTATE_AT → session is NOT rotated."""
    async def fake_engine(**kwargs):
        yield {"type": "text", "text": "hi"}
        yield {"type": "result", "session_id": "sess-small", "context_tokens": 50000}

    ctx = _make_ctx(tmp_path, project_dir, run_engine=fake_engine)
    with patch.object(_webapp, "CONTEXT_ROTATION", True), \
         patch.object(_webapp, "CONTEXT_ROTATE_AT", 280000), \
         patch.object(_webapp, "_QUEUE", {}), \
         patch.object(_webapp, "_build_handoff", AsyncMock(return_value="x")):
        app = _make_app(ctx)
        client = await aiohttp_client(app)
        resp = await client.post("/api/projects/myproject/chat",
                                 json={"prompt": "small"}, headers=_auth_headers(ctx))
        assert resp.status == 200
        await _read_sse(resp)
    assert ctx["sessions"].get(SESSION_KEY) == "sess-small", "below-cap session must survive"


async def test_auto_rotate_disabled_by_toggle(aiohttp_client, tmp_path, project_dir):
    """CONTEXT_ROTATION=False → no rotation even above the cap."""
    async def fake_engine(**kwargs):
        yield {"type": "text", "text": "hi"}
        yield {"type": "result", "session_id": "sess-off", "context_tokens": 300000}
    ...
    with patch.object(_webapp, "CONTEXT_ROTATION", False), \
        ...
        client = await aiohttp_client(app)
        resp = await client.post("/api/projects/myproject/chat",
                                 json={"prompt": "big but disabled"}, headers=_auth_headers(ctx))
        assert resp.status == 200
        await _read_sse(resp)
    assert ctx["sessions"].get(SESSION_KEY) == "sess-off", "rotation OFF → session must survive"
