...
main() {
  ...
  # Systemd unit name — OSS default "cardloop"; overridable via CARDLOOP_SERVICE (env or .env).
  local service="${CARDLOOP_SERVICE:-}"
  if [ -z "$service" ] && [ -f .env ]; then
    # `|| true`: a no-match grep exits 1, which under `set -e`+pipefail would
    # silently abort the whole updater before it ever fetches — and most OSS
    # instances DON'T set CARDLOOP_SERVICE, so this hit every default install.
    service="$(grep -E '^[[:space:]]*CARDLOOP_SERVICE=' .env 2>/dev/null | tail -n1 | cut -d= -f2- | awk '{print $1}' | tr -d '"')" || true
  fi
  ...
  if systemctl cat "${service}.service" >/dev/null 2>&1; then
    ...
    systemctl --no-pager -n 5 status "$service" || true
  else
