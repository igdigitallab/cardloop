...
- **Фаза 0 (F0) ✅ done**, **Фаза 3 авто-запуск (F1) ✅ done** (см. выше). Осталось: **Фаза 2 (C1)** — стрим-чат (SSE) в кокпите, сессии общие с TG (переиспользует `run_engine`, добавит live-прогресс карточкам вместо поллинга).
