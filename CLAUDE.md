...
- **Фаза 1 (готово):** read-only — список проектов + git-health, табы Обзор / README (`/api/projects/{id}/readme`, перебор имён файла) / CLAUDE.md / Specs (из vault `01-Projects/<X>/specs/` + локальной `<cwd>/specs/`, дедуп; агент часто пишет локально) / Активность (audit-лог).
...
- **Статус фаз:** F0 ✅ · F1 ✅ · C1 чат ✅ · проводник ✅ · ресайз+сессии ✅ · история чата ✅ · шина live-прогресс ✅. В бэклоге: «Контекст сессии» (файлы что сессия читала), «Память проекта» (memory/ файлы). Дальше по roadmap: O1 Timeline, C2 гейт, автономный контур.
