...
- **`activeId + GLOBAL_FILES_ID` в cleanup-эффекте (fix 2026-05-30).** Эффект чистки проектов `setActiveId(prev => valid.has(prev) ? prev : null)` сбрасывал `'__global__'` (не является ID проекта) → глобальный файловый таб исчезал при загрузке проектов. Фикс: явная проверка `prev === GLOBAL_FILES_ID || (prev && valid.has(prev)) ? prev : null`.
- **`GlobalFilesTab` держать mounted через `display:none`, не `&&` (fix 2026-05-30).** Условное монтирование уничтожает state дерева (expand/scroll/выделение) при каждом переключении таба. Паттерн: `{globalFilesOpen && <div style={{display: activeId===GLOBAL_FILES_ID?'flex':'none'}}>}`. Так же как и для project-views.
- **`TASKS.md` гонка записи (fix 2026-05-30).** GET /tasks читает файл → агент пишет → GET перезаписывает стариком. Фикс: перед записью перечитать файл (`tp.read_text()`), сравнить с `raw` — если изменился снаружи, пропустить запись. Шаблон: `if current == raw: tp.write_text(canon)`.
- **Sidebar loading flash (fix 2026-05-30).** `setProjectsLoading(true)` на каждом 15с-полле показывал «Загрузка…» в сайдбаре. Фикс: `projectsLoadedRef` — флаг первой загрузки; при повторных поллах loading не ставить. Плюс стабильное сравнение: `setProjects(prev => JSON.stringify(prev)===JSON.stringify(res.projects) ? prev : res.projects)` — не триггерит cascade эффектов при неизменных данных.
- **`busActiveRef` на рефреше (fix 2026-05-30).** После перезагрузки страницы `busActiveRef=false` на mount; `run_start` уже ушёл → все последующие SSE-события фильтровались, индикатор прогресса не появлялся. Фикс: в mount-эффекте `ChatTab` параллельно `sessionHistory` вызывать `GET /api/projects/{id}/running`; если `running=true` — сразу ставить `busActiveRef=true` и `setRun(...)`.
