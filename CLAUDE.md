...
- `features/board_janitor/` — the Review column's only automatic exit. `logic.py` holds the pure
  policy (a card is archived ONLY on evidence: its own run settled + project tests green — a
  missing test signal is never permission), `loop.py` sweeps every `BOARD_JANITOR_INTERVAL_SEC`,
...
- `roles.py` + `roles/builtin/*.md` — spec-091 declarative sub-agent roles: a three-tier file registry (builtin ships in git → global `$CARDLOOP_ROLES_DIR`/`~/.claude-ops/roles` → project `<cwd>/.claude-ops/roles`), whole-file override by name, cockpit-editable via the Agents tab. `engine.py` compiles it into the Task/Workflow roster; `DEFAULT_AGENTS` there is only the broken-install fallback when zero role files resolve.
  ...
  9-planted-defect fixture (`docs/internal/specs/spec-091-agent-roles/reports/EVAL-results.md`):
  a three-reviewer panel tied a single reviewer at 9/9 for ~40% more tokens, while
...
- `web/src/lib/popout.ts` + `components/PopoutApp.tsx` — pop-out project windows for multiple monitors (⧉ in the project header / sidebar menu). `/?popout=<id>[&tab=]` makes `main.tsx` mount `PopoutApp` INSTEAD of `<App/>`: one `ProjectView` (left pane + chat + divider), nothing else. ⚠️ Three omissions are the design, not gaps — do not "complete" the pop-out: (1) no global `/api/activity-stream` and its project stream only while visible — over plain HTTP/1.1 every window of the browser shares ONE 6-connection pool per host; (2) never hydrate/save `ui_state` — the server keeps ONE `"default"` layout and a pop-out write wipes the main window's tabs; (3) its own localStorage keys (`popoutKey`). Re-focusing uses `window.open('', name)` — `window.open(url, name)` re-navigates the open window and kills its streams. e2e: `tests/e2e/test_popout_window.py`. Serve the cockpit over HTTPS (any HTTP/2 reverse proxy) to lift the 6-connection cap for good.
...
- ⚠️ **A "frozen chat" / "dead browser pane" is often an OOM kill, not a UI bug.** Every live client is a whole CLI subprocess (~0.4-0.6 GB RSS, more with sub-agents), so `LIVE_CLIENT_MAX` bounds the COUNT while memory keeps climbing: on ops the cgroup hit `MemoryMax` twice (2026-08-26, 2026-08-27), the kernel killed a `claude` child mid-turn and systemd restarted the service — with `systemctl is-active` still saying `active` the whole time. `doctor` now escalates `MemoryCurrent` (⚠ ≥75%, ✗ ≥90%) and reports the cgroup's `oom_kill` counter plus the unit restart count; check those BEFORE debugging the frontend. Knobs: `LIVE_CLIENT_MAX`, `LIVE_CLIENT_TTL_SEC`, `LIVE_CLIENT_MEM_GUARD` (fraction of the cgroup limit above which idle clients are LRU-evicted before a new one connects). ⚠️ The cockpit's own python (~420 MB) is the largest single process in the cgroup, so the kernel used to kill IT (2026-09-24: all 7 live chats died, one silently for 10 h). `_oom_shield_loop` raises every other process to `OOM_CHILD_SCORE_ADJ` (default 500) so an OOM takes one agent instead; `OOMPolicy=continue` only helps once the victim is a child.
- **Tests: `venv/bin/python -m pytest tests/`** (3247 passed + 7 skipped as of 2026-09-11, ~97 s; should be green). ⚠️ ONLY via the venv — it has `pytest-aiohttp` (requirements-dev.txt); the system `python` does NOT, so ~237 endpoint tests fall into a false `error`. Do not trust such a run and do NOT rewrite tests to fit it.
...
- ⚠️ **Model-alias ground truth (`tools/verify_model_aliases.py`):** the UI label comes from the LIVE `/v1/models` listing, but the cockpit sends the BARE alias (`opus`) to the SDK and the **bundled CLI** decides what that alias means — a stale bundle silently runs an older model (`is_error=False`). Run `venv/bin/python tools/verify_model_aliases.py` (exit 1 = mismatch) or `venv/bin/python -m pytest tests/test_model_aliases.py -m aliases` after ANY model release, and bump the `claude-agent-sdk` floor when it flags. The offline half — the three static label spots staying in lockstep — is asserted by `test_static_label_spots_stay_in_sync` in the default run. A daily cron (08:25) runs `--watch`: ONE read-only `/v1/models` GET and zero model calls unless a model id we have never seen appears (state: `data/model-release-seen.json`), and only then does it probe. Mismatch → exit 1 (healthchecks alert) + `data/inbox/model-alias-mismatch.txt`. ⚠️ **Bumping the SDK is not always a possible fix.** The CLI also gates which ids the API will serve at all — a model newer than the CLI is a hard 400 ("Claude Code 2.1.276 does not support this model; version 2.1.280 or newer is required"), by alias AND by explicit id — and models ship before any `claude-agent-sdk` release bundles a CLI that can run them (Opus 5.5: released 2026-09-21, newest SDK bundled 2.1.277). `CLAUDE_CLI_PATH` (`runtime.cli_path()`, resolved PER RUN — never snapshot it into a module constant, bot.py imports webapp before it parses `.env`; fed to `ClaudeAgentOptions(cli_path=...)`) points the whole cockpit at a standalone CLI — `npm i -g --prefix ~/.npm-global @anthropic-ai/claude-code@<ver>` — for that window; unset or not-an-executable falls back to the bundle. `verify_model_aliases` and `doctor` both report the ACTIVE binary, not the bundle.
...
- **`claude-agent-sdk` >= 0.2.150 is required (history: >=0.2.96 fable/spec-017, >=0.2.110, >=0.2.129, >=0.2.143 spec-085, >=0.2.144, >=0.2.149, >=0.2.150 Fable 5.1/spec-088).** An old SDK silently substitutes a different model with no error (`is_error=False`). **The SDK's BUNDLED CLI determines model-alias resolution** — old bundles resolve `sonnet`→`claude-sonnet-4-6` and lack Sonnet 5; keep the SDK fresh after model releases. After recreating the venv: `pip install -U "claude-agent-sdk>=0.2.150"`. Symptoms of a stale bundle: sub-agents billed on a previous-generation model, or session replies "issue with the selected model".
