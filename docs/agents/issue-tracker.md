...
Issues and specs for this repo are **Cardloop kanban cards**, not GitHub Issues. The board
...
- Specs (you may know a spec as a PRD) live in `docs/internal/specs/spec-NNN-*.md`
  (gitignored, operator-local).

## When a skill says "publish to the issue tracker"

Add a `- [ ] <text>` line under `## Backlog` in `TASKS.md`. Do **not** use `gh issue create` —
this repo does not track work in GitHub Issues.

## When a skill says "fetch the relevant ticket"

Read the card text from `TASKS.md` (match by the `ops:ID` marker or the card text). The operator
normally references a card by its short id or wording.
