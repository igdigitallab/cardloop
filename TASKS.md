...
- [ ] ⭐ Рефакторинг Ф2: UI три зоны (Шапка/Обзор/Настройки), вкладки 9→7 (spec-011)
  > Шапка: модель-select + git-чип + Sync постоянно + индикатор агента + счётчик инцидентов + кнопка «Прогнать тесты». Обзор: убрать тумблеры self_heal/notify и плашку «авто-скан 5 мин» (read-only дашборд). Настройки: влить «Ключи» секцией. «Лента»→«Активность». Дедуп модели (3 места) и projectHealth-fetch. Снос мёртвого фронта (api.readme/specs/activity, DisabledTab). Готово: ноль дублей, 7 вкладок, npm build зелёный.
