...
- [ ] Live-trace: видеть что делают субагенты внутри сессии + не терять историю/таймер после обновления страницы → spec-035 <!--ops:847153-->
  > Учтено и расширено в specs/spec-035-live-trace.md: мультизритель (телефон/2-й браузер), реплей по реконнекту (Last-Event-ID), серверный таймер, дорожка субагентов с last_tool_name. Запускать ПОСЛЕ коммита spec-034 — общие файлы webapp.py/bot.py/ChatTab.tsx.
