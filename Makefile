...
service:        ## Render + install the systemd unit, then enable & start it
	@sed "s/__USER__/$$(whoami)/g" claude-ops-bot.service.template \
		| sudo tee /etc/systemd/system/claude-ops-bot.service >/dev/null
	sudo systemctl daemon-reload
	sudo systemctl enable --now claude-ops-bot
