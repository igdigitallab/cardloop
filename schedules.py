...
async def _emit_schedule_incident(
    ...
    Uses the record's project or falls back to 'cardloop' (server-level).
    Anti-flood: dedup via _report_incident's own debounce + 24h dismissed_incidents.
    """
    try:
        import webapp as _wa
        project_id = record.get("project") or "cardloop"
...
async def investigate_schedule(ctx: dict, record_id: str) -> dict:
    ...
    The card is created in the relevant project (or 'cardloop' if no project).
    Returns {"card_id": "..."}.
    """
    import webapp as _wa
