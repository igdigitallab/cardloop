...
- `CLAUDE.md` (репо): личный TG ID, путь к userbot-сессии `…/secrets/tg.session`,
  упоминание OPSEC-проекта `khronika-portal`.
- `bot.py: _REG_RAW` (~108–130): полный список личных проектов Игоря (rightforms,
  khronika, even-g2, …) вшит как static registry.
...
- `specs/spec-011…`: упоминание `khronika-portal` (OPSEC).

### Обязательно, но не блокер (сломает чужую инсталляцию)
- `claude-ops-bot.service`: `User=igor`, `Group=igor`, все пути `/home/igor/…`,
  `PATH=/home/igor/.npm-global/…`.
- `web/src/tabs/LogsTab.tsx:61,64,66,67`: захардкоженный `/home/igor/claude-ops-bot/
  data/topics.json` + имя `igor` в prompt-тексте для агента.
- `README.md` / `ARCHITECTURE.md` / `CONTRIBUTING.md`: `@ziraclaudebot`, `Zira777ru`,
  `claude-ops.coscore.us`, `192.168.0.114:8787`, `/home/igor/…`.
- `templates/` + `templates/reference/`: «Игорь», `/home/igor/`, `~/vault/`,
  `firecrawl.coscore.us`, имена проектов как примеры.
...
2. **`VAULT_PROJECTS` → env.** Добавить `VAULT_PROJECTS` в `.env` Игоря со значением
   `/home/igor/vault/01-Projects`; код читает env, дефолт пусто = фича отключена.
...
4. **`LogsTab.tsx`** — убрать `/home/igor/...` и имя `igor` из prompt-текста (динамика/
...
8. `README.md`, `ARCHITECTURE.md`, `CONTRIBUTING.md`: заменить `@ziraclaudebot` →
   `@YOUR_BOT`, `Zira777ru` → `YOUR_GITHUB`, `claude-ops.coscore.us` → `YOUR_DOMAIN`,
   `192.168.0.114:8787` → `localhost:8787`, `/home/igor/…` → относительные/`$HOME`.
9. `templates/` + `templates/reference/`: «Игорь» → «оператор/you», `/home/igor/` →
   `$HOME/`, `~/vault/` → обобщить, `firecrawl.coscore.us` → `YOUR_FIRECRAWL_URL`,
...
10. `CLAUDE.md` (репо): убрать личный TG ID, путь к userbot-сессии, OPSEC-имя
    `khronika-portal`. **Сохранить технические gotchas** — это операционный гайд агентов,
    не выхолащивать. Личные имена «Игорь» в gotchas → «оператор», где не теряется смысл.
...
15. Grep-аудит трекаемых файлов: не осталось `282311426`, `coscore.us`,
    `192.168.`, `Zira777ru`, `/home/igor`, `ziraclaudebot` (кроме самого этого спека
