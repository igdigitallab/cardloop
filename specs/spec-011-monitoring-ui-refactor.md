...
**Чистка локальная:**
- `scripts/khronika-web-logs.sh` — untracked локальный wrapper (НЕ в git, не утечка). Решить: оставить + `scripts/` в `.gitignore` (на случай publish), или перенести в vault. **S**
...
- модель — **select** (вместо текста-чипа); git-чип (ветка/dirty/unpushed) + кнопка **Git sync постоянно** (убрать условие `gitNeedsSync`); индикатор «идёт агент» (из `useProjectActivity` run_start/run_end); счётчик инцидентов (`project.incidents` → чип, клик в Доску); кнопка **«Прогнать тесты»** (перенос `TestRunner` из Обзора). Все — **S**, кроме общей сборки шапки **M**.
...
**Дедуп:**
- модель: 3 места (Overview инфо-карточка / Settings / селектор в `ChatTab:~529`) → дом = Шапка + Настройки; из чата убрать или сделать read-only с подсказкой. РЕШЕНИЕ Игоря.
...
## Решения Игоря до сборки (3 развилки)
1. **CLAUDE.md** — отдельная вкладка или в «Файлы»?
2. **Модель** — единый дом (Шапка+Настройки) и убрать из чата, или оставить в чате?
3. **scripts/** — gitignore (защита перед publish) или перенос khronika-скрипта в vault?
