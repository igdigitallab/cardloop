...
> **Статус: ✅ ГОТОВО (реализовано 2026-06-03/04).** Все фазы Ф0→Ф3 закрыты, задеплоены, pytest зелёный (529→589 с покрытием Ф3). Источник: аудит 4 агентами 2026-06-03 + серия решений оператора.
> Истина по коду — `ARCHITECTURE.md` / `docs/API.md`. Этот файл — исторический план; что реально сделано — см. коммиты ниже.
>
> **Реализация (коммиты):**
> - **Ф0** `87e49f9` — running-утечка, watchdog, двойной pop, мёртвые импорты, дубль `_NEW_PROJECT_PROMPT`, `req.json` guards, **глобальный error_middleware**; сторонний скрипт вынесен в отдельный проект.
...
**example-project — вынести из claude-ops-bot (РЕШЕНО):**
- `scripts/example-web-logs.sh` (untracked, не в git) → перенести туда где ей место: vault/Homelab или log_cmd самого проекта. ClaudeOps и example-project — НЕсвязанные проекты, чужую специфику в репо оркестратора не держим. После переноса — поправить `log_cmd` в topics.json, чтобы логи в кокпите не отвалились. **S**
...
3. **example-project** — вынести из claude-ops-bot ПОЛНОСТЬЮ (см. Ф0). ClaudeOps и example-project — несвязанные проекты.
