...
> **Статус: ✅ ГОТОВО (реализовано 2026-06-03/04).** Все фазы Ф0→Ф3 закрыты, задеплоены, pytest зелёный (529→589 с покрытием Ф3). Источник: аудит 4 агентами 2026-06-03 + серия решений Игоря.
> Истина по коду — `ARCHITECTURE.md` / `docs/API.md`. Этот файл — исторический план; что реально сделано — см. коммиты ниже.
>
> **Реализация (коммиты):**
> - **Ф0** `87e49f9` — running-утечка, watchdog, двойной pop, мёртвые импорты, дубль `_NEW_PROJECT_PROMPT`, `req.json` guards, **глобальный error_middleware**; Khronika-скрипт вынесен в `khronika-portal/`.
> - **Ф1** `793dd13` — сканер тестов OFF (только `log_cmd`), стандартная строка `UNHANDLED` + умный дедуп, capability-health (`_detect_error_handler`), шаблоны `## Error Handler` + `## ClaudeOps conformance`.
> - **Ф2** `772d882` — UI три зоны (Шапка/Обзор/Настройки), вкладки 9→8 («Лента»→«Активность», «Ключи»→секция), модель только в чате (реальные версии), дедуп health-fetch, снос мёртвого фронта.
> - **Ф3** `9208c4b` — unify `api_project_logs`, ленивый `_usage_lock`, импорты в топ, `_spawn_bg` (GC), +50 тестов, прунинг orphan-сайдкаров.
> - **Постфактум** `89a98b9` (SSE benign-disconnect не плодит инциденты), `f2c9bf8` (`err-` card_id валиден), `1a8960d` (кнопки: «Скан» убрана, «Тесты» вывод). Преемник — **spec-012** (инциденты в реальном времени).
> - Ревью каждой фазы: 2 независимых агента-ревьюера, замечания (вкл. несколько BLOCKER) устранены до коммита.
