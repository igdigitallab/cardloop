...
1. 📱 **Mobile-first PWA.** Not "responsive" — *installable*. Add it to your home screen and run agents
   from your phone: live SSE streaming, reconnect on wake, safe-area, pinch-zoom. The phone is the primary surface.
2. 🤖 **Claude-first, subscription auth.** Runs on your existing Claude Max/Pro subscription via
   `claude login` — **no API key, no per-token bill.** Zero marginal cost per card.
3. 🗂️ **Cards that code.** A card's task text *becomes* the agent prompt. No dispatch layer — moving a
   card to In Progress runs the engine, attaches the diff, and moves the card to Review.
...
- **Kanban-as-memory** — `TASKS.md` in each repo *is* the board. Sections are columns, lines are cards.
...
- **Production-grade internals** — 1400+ tests, encrypted secret vault, C2 destructive-command gate,
  double path-traversal defence, single-operator auth (web password + optional TOTP 2FA).
...
The lifecycle: **you add a card → drag it to In Progress → the engine runs the task → result + git diff
are attached → the card lands in Review (or Failed) → you get a ping.** Move a card, and the agent picks it up.
...
- **Agents run with `bypassPermissions` — full host access by design.** They edit files, run git, and
  ...
  autonomous agent. A C2-style gate guards the most destructive commands, but the model is "trusted
  operator," not "sandboxed."
...
Python 3.11 · aiohttp · Claude Agent SDK · React 18 · Vite · TypeScript · systemd · pytest
