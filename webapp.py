...
Runs in the same process/loop as the launcher (bot.py).
...
# ── Cost-management thresholds (DECOUPLED from the hard model window above) ──────────────────────
# These are NOT about running out of context — they are about RE-BILL COST on the tail.  A
...
ROTATION_SUMMARY_PROMPT = (
    ...
    "## Where we stopped\n"
    "(Current state + the immediate next step — this section is most important.)\n\n"
    "## Open questions / waiting on operator\n"
    "(Anything blocked, unresolved, or needing a decision.)\n\n"
    "## Key decisions (and why)\n"
    "(Choices made and the rationale, however brief.)\n\n"
    "## Done this session\n"
...
def _emit_board_event(session_key: str, *, event: str, card_id: str, title: str = "",
                      ...
                      summary: str = "") -> None:
    """spec-052 Phase 2: surface a board change in the project's chat.
    Reuses the per-project SSE bus. persist=True records it to the timeline JSONL
    (the /timeline audit panel), not the chat — board strips are live-only in chat
    and do not survive a full chat reload; the frontend dedup is defensive."""
    if not session_key:
        return
    payload = {
        "kind": "board_event", "event": event, "card_id": card_id,
        "title": (title or "")[:120], "column_from": column_from,
        "column_to": column_to, "severity": severity,
        "summary": (summary or "")[:240], "ts": time.time(),
    }
    _bus_publish(session_key, payload, persist=True)
    # spec-052 Phase 7: buffer actionable strips so they survive reload / inactive tab /
    # mid-stream (re-injected by api_project_live). run_start/run_end are transient
    # banner-drivers, not feed notifications — don't buffer them.
    if event in ("moved", "incident", "reconcile"):
        buf = _recent_board_events.get(session_key)
        if buf is None:
            buf = deque(maxlen=_BOARD_EVENTS_MAXLEN)
            _recent_board_events[session_key] = buf
        buf.append(payload)


def _sync_specs(session_key: str, cwd: str, spec_ids) -> None:
    """spec-052 Phases 5-6: regenerate the generated ## Tasks mirror for each
    touched spec, and announce in the project chat a spec that just reached
    all-cards-done (auto-close). Best-effort — never raises into the caller."""
    ids = {s for s in (spec_ids or []) if s}
    if not ids:
        return
    try:
    ...
    for sid in ids:
        try:
            res = sync_spec_mirror(cwd, sid)
        except Exception as e:
            print(f"[spec-mirror] sync failed for spec {sid}: {e}")
            continue
        if res is None:
            print(f"[spec-mirror] spec {sid} linked on a card but spec-{sid}-*.md not found")
        elif res.get("newly_closed"):
            extra = " · status → shipped" if res.get("status_stamped") else ""
            _emit_board_event(
                session_key, event="reconcile", card_id=f"spec-{sid}",
                title=f"spec-{sid} complete",
                severity="success",
                summary=f"All {res['total']} cards done — spec auto-closed{extra}",
            )
...
# Per-session set of monitor ids the operator dismissed. A raw bg-bash shell keeps emitting tool
# deltas after dismissal (its natural completion is invisible to the engine), which would otherwise
# re-create the row via _monitor_update — so dismissed ids are suppressed until rotate/reset.
_monitors_dismissed: dict[str, set] = {}
_MONITORS_DISMISSED_MAX = 200  # cap per session — bound the suppression set
...
def _monitor_update(session_key: str, delta: dict, only_existing: bool = False) -> None:
    ...
    try:
        ...
        if not mid:
            return
        if mid in _monitors_dismissed.get(session_key, ()):  # operator dismissed — stay gone
            return
...
def _monitor_dismiss(session_key: str, mid: str) -> bool:
    ...
    bucket = _monitors.get(session_key)
    # Suppress this id even mid-flight: a still-running bg-bash keeps emitting deltas that would
    # otherwise re-create the row. Suppression is cleared on session rotate/reset (_monitors_clear).
    dset = _monitors_dismissed.setdefault(session_key, set())
    dset.add(mid)
    if len(dset) > _MONITORS_DISMISSED_MAX:
        dset.discard(next(iter(dset)))
    existed = bool(bucket and mid in bucket)
    if bucket:
        bucket.pop(mid, None)
        if not bucket:
            _monitors.pop(session_key, None)
...
# board events so api_project_live can re-inject them into the chat feed on hydration.
_recent_board_events: dict[str, "deque[dict]"] = {}
_BOARD_EVENTS_MAXLEN = 30          # cap per session
_BOARD_EVENTS_RETAIN_SEC = 1800    # 30 min recency window on read


def _recent_board_events_for(session_key: str) -> list[dict]:
    """Recent actionable board events for a session, newest-relevant, within the window."""
    buf = _recent_board_events.get(session_key)
    if not buf:
        return []
    cutoff = time.time() - _BOARD_EVENTS_RETAIN_SEC
    return [e for e in buf if (e.get("ts") or 0) >= cutoff]


def _board_events_clear(session_key: str) -> None:
    """Drop a session's buffered board strips (called on rotate/reset — stale strips from
    the old session must not re-hydrate into the fresh chat). Mirrors _monitors_clear."""
    _recent_board_events.pop(session_key, None)


def _live_turn_create(session_key: str, model: str, prompt: str = "") -> dict:
...
#
# These are the built-in default prompt templates shipped with every Cardloop
...
# ── Spec-061: nested folder paths ("Business/Clients/VIP") ────────────────────

def _normalize_group_path(raw):
    """Validate + normalize a folder path. '/' is the separator; each segment
    must be non-empty after .strip(). Returns the normalized path (segments
    stripped and rejoined) or None if any segment is empty/whitespace."""
...
def _group_ancestors(path: str):
    """Ancestor paths of `path`, shallowest first, INCLUDING `path` itself.
...
def _path_allowlist_check(cwd: str, ctx: dict, _home_override: str | None = None) -> str | None:
    ...
    try:
        ...
        if here_real:
            if real == here_real:
                return "cwd is the Cardloop server directory"
            if here_real.startswith(real + os.sep):
                return "cwd is an ancestor of the Cardloop server directory"
...
# ─────────────────────────────────────────────────────────────────────────
# Version & self-update (spec-047 workstream A)
#
# Source of truth = git tags / commits. /api/version is cheap (local git only)
# and never blocks on the network — a throttled background `git fetch` keeps the
# cache fresh. /api/update spawns a DETACHED updater that applies the update and
# restarts via restart-self.sh — never an inline `systemctl restart` from this
# process (the self-suicide hazard; see GOTCHAS "self-restart = suicide").
# ─────────────────────────────────────────────────────────────────────────

_VERSION_FETCH_TTL = 1800.0          # throttle `git fetch` to once per 30 min
_version_fetch_at = 0.0              # wall-clock of the last fetch attempt
_version_fetch_lock = asyncio.Lock()


def _git(here: Path, *args: str, timeout: float = 5.0) -> tuple[int, str]:
    """Run a git command in `here`; return (returncode, stripped stdout)."""
    try:
        r = subprocess.run(
...
def _version_info(here: Path) -> dict:
    """Cheap, local-git only — never hits the network."""
    if _git(here, "rev-parse", "--git-dir")[0] != 0:
        return {
            "current": "unknown", "latest": None, "behind": 0,
            "update_available": False, "channel": "unknown",
            "can_self_update": False,
            "reason": "not a git checkout (copied folder?) — reinstall via git clone",
            "update_status": _read_update_status(here),
        }
    current = _git(here, "describe", "--tags", "--always", "--dirty")[1] or "unknown"
    branch = _git(here, "rev-parse", "--abbrev-ref", "HEAD")[1] or "HEAD"
    has_origin = _git(here, "remote", "get-url", "origin")[0] == 0
    dirty = bool(_git(here, "status", "--porcelain")[1])

    behind = 0
    latest = None
    if has_origin and branch != "HEAD":
        rc, out = _git(here, "rev-list", "--count", f"HEAD..origin/{branch}")
        if rc == 0 and out.isdigit():
            behind = int(out)
        rc, out = _git(here, "tag", "--sort=-v:refname", "--merged", f"origin/{branch}")
        if rc == 0 and out:
            latest = next((ln.strip() for ln in out.splitlines() if ln.strip()), None)

    has_systemd = shutil.which("systemctl") is not None
    reason = None
    if not has_origin:
        reason = "no 'origin' remote — can't check for updates"
    elif branch == "HEAD":
        reason = "detached HEAD — check out a branch first"
    elif dirty:
        reason = "working tree has local changes — commit or stash first"
    elif not has_systemd:
        reason = "no systemd (Docker?) — update with: git pull && docker compose up --build"
    can_self_update = reason is None
    update_available = bool(can_self_update and behind > 0)
    return {
        "current": current, "latest": latest, "behind": behind,
        "update_available": update_available, "channel": branch,
        "can_self_update": can_self_update, "reason": reason,
        "update_status": _read_update_status(here),
    }


def _read_update_status(here: Path) -> dict | None:
    """Last detached-updater result, if any (written by scripts/self-update.sh)."""
    try:
        f = here / "data" / "update-status.json"
        if f.is_file():
...
async def _do_fetch(here: Path) -> None:
    """Throttled, network-bound `git fetch` run off the event loop."""
    global _version_fetch_at
    async with _version_fetch_lock:
        if time.time() - _version_fetch_at < _VERSION_FETCH_TTL:
            return
        _version_fetch_at = time.time()
        branch = _git(here, "rev-parse", "--abbrev-ref", "HEAD")[1] or "HEAD"
        if branch == "HEAD":
            return
        await asyncio.to_thread(
            _git, here, "fetch", "--quiet", "--tags", "origin", branch,
        )
...
async def api_update(req: web.Request) -> web.Response:
    ctx = req.app["ctx"]
    here: Path = ctx.get("HERE") or Path(__file__).resolve().parent
    info = _version_info(here)
    if not info["can_self_update"]:
        return web.json_response({"error": info.get("reason") or "cannot self-update"}, status=409)
    if not info["update_available"]:
        return web.json_response({"status": "up_to_date"}, status=200)
    updater = here / "scripts" / "self-update.sh"
    if not updater.is_file():
        return web.json_response({"error": "updater script missing (scripts/self-update.sh)"}, status=500)
    try:
        # Detached (new session) so it survives the restart it will trigger.
        subprocess.Popen(
            ["bash", str(updater)], cwd=str(here),
            stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
            start_new_session=True,
        )
    except Exception as e:
        return web.json_response({"error": f"failed to start updater: {e}"}, status=500)
...
async def api_project_groups_create(req: web.Request) -> web.Response:
    ...
    `name` may be a full path ("A/B/C"). Auto-creates missing ancestors, appending
    them in ancestor order to the end of `groups`.  Idempotent: already-exists → 200
    with current state.  Returns full state."""
    ctx = req.app["ctx"]
    try:
        body = await req.json()
    except Exception:
        return web.json_response({"error": "invalid json"}, status=400)
    name = _normalize_group_path(body.get("name"))
    if name is None:
        return web.json_response({"error": "name must be a non-empty path"}, status=400)
    groups_data = _load_groups(ctx)
    changed = False
    for ancestor in _group_ancestors(name):
        if ancestor not in groups_data["groups"]:
            groups_data["groups"].append(ancestor)
            changed = True
    if changed:
        _save_groups(ctx, groups_data)
...
async def api_project_groups_rename(req: web.Request) -> web.Response:
    ...
    Prefix-aware SUBTREE cascade — covers both rename-leaf and move/re-parent.
    Affected = `from` plus every g.startswith(from + "/"); each affected g maps to
    `to + g[len(from):]`.  Auto-creates missing ancestors of `to`.  Remaps
    assignments the same way.  Returns full state."""
    ctx = req.app["ctx"]
    try:
        body = await req.json()
    except Exception:
        return web.json_response({"error": "invalid json"}, status=400)
    from_name = _normalize_group_path(body.get("from"))
    to_name = _normalize_group_path(body.get("to"))
    if from_name is None:
        return web.json_response({"error": "from must be a non-empty path"}, status=400)
    if to_name is None:
        return web.json_response({"error": "to must be a non-empty path"}, status=400)
    groups_data = _load_groups(ctx)
    if from_name not in groups_data["groups"]:
        return web.json_response({"error": f"group '{from_name}' not found"}, status=400)
    # No-op rename is idempotent.
    if to_name == from_name:
        return web.json_response(groups_data)
    # Reject moving a folder into itself/its own descendant — would remap paths onto
    # an ever-deepening prefix and orphan the subtree.  The UI already excludes this
    # (Move-folder hides self+descendants, inline rename only edits the leaf); this
    # hardens the public endpoint against a direct call.
    if to_name.startswith(from_name + "/"):
        return web.json_response(
            {"error": "cannot move a folder into itself or its descendant"}, status=400
        )
    # Subtree affected by the rename: the folder itself + all descendants.
    prefix = from_name + "/"
    affected = {g for g in groups_data["groups"] if g == from_name or g.startswith(prefix)}

    def _remap(path: str) -> str:
        return to_name + path[len(from_name):]

    # Collision: any new path lands on an existing group OUTSIDE the affected set.
    for g in affected:
        new_path = _remap(g)
        if new_path in groups_data["groups"] and new_path not in affected:
            return web.json_response({"error": f"group '{new_path}' already exists"}, status=400)
    # Rebuild groups preserving order, replacing each affected g with its new path.
    new_groups = [_remap(g) if g in affected else g for g in groups_data["groups"]]
    # Auto-create missing ancestors of `to` (so a move lands under an existing chain).
    for ancestor in _group_ancestors(to_name):
        if ancestor not in new_groups:
            new_groups.append(ancestor)
    groups_data["groups"] = new_groups
    # Remap every assignment whose value is in the moved subtree.
    groups_data["assignments"] = {
        pid: (_remap(label) if (label == from_name or label.startswith(prefix)) else label)
...
async def api_project_groups_delete(req: web.Request) -> web.Response:
    ...
    SUBTREE cascade — affected = `name` + every descendant (startswith name + "/").
    Removes all affected from groups and unassigns every project pointing into the
    subtree.  Idempotent if absent.  Returns full state."""
    ctx = req.app["ctx"]
    try:
        body = await req.json()
    except Exception:
    ...
    groups_data = _load_groups(ctx)
    prefix = name + "/"
    affected = {g for g in groups_data["groups"] if g == name or g.startswith(prefix)}
    if affected:
        groups_data["groups"] = [g for g in groups_data["groups"] if g not in affected]
        groups_data["assignments"] = {
            pid: label for pid, label in groups_data["assignments"].items()
...
async def api_project_epic_specs(req: web.Request) -> web.Response:
    """GET /api/projects/{id}/epic-specs — spec-049 B / spec-059 Move 1: the project's
    specs (docs/internal/specs) as epics, each annotated with its linked board cards
    and progress. Read-only."""
    ctx = req.app["ctx"]
    project = _find_project_by_id(ctx, req.match_info["id"])
    if project is None:
        return web.json_response({"error": "project not found"}, status=404)
    from spec_epics import build_epic_specs
    return web.json_response({"specs": build_epic_specs(project["cwd"])})


async def api_project_epic_spec_content(req: web.Request) -> web.Response:
    """GET /api/projects/{id}/epic-specs/{name} — markdown of one epic spec file."""
    ctx = req.app["ctx"]
    project = _find_project_by_id(ctx, req.match_info["id"])
    if project is None:
        return web.json_response({"error": "project not found"}, status=404)
    from spec_epics import get_epic_spec_content
    content = get_epic_spec_content(project["cwd"], req.match_info["name"])
    if content is None:
        return web.json_response({"error": "spec not found"}, status=404)
    return web.json_response({"name": Path(req.match_info["name"]).name, "content": content})
...
def _parse_log_errors(log_text: str, source: str = "log") -> list[dict]:
    ...
    for m in _PY_TRACEBACK_RE.finditer(log_text):
        ...
        # Noise filter: benign transient exceptions (e.g. transient network errors) —
...
async def _ingest_errors_to_board(cwd: str, name: str, errors: list[dict],
                                   session_key: str = "") -> tuple[int, int]:
    ...
    spec-052 Phase 2: emits board_event(incident) for each newly created card when session_key set."""
    if not errors:
        return (0, 0)
    ...
    async with lock:
        ...
        added = 0
        updated = 0
        # spec-052: collect new cards for board_event emission after write
        new_incident_events: list[dict] = []
        ...
        for err in errors:
            ...
            if h in existing:
                ...
                card_title = _incident_title(err)
                cols["failed"].append({
                    "id": f"err-{h}",
                    "text": card_title,
                ...
                if session_key:
                    new_incident_events.append({"card_id": f"err-{h}", "title": card_title})
                added += 1

        if added or updated:
            _save_board(cwd, name, preamble, cols)

        # spec-052 Phase 2: emit one board_event per new incident (after board write)
        for ev in new_incident_events:
            _emit_board_event(
                ...
                column_to="failed",
                severity="error",
                summary=f"New incident: {ev['title']}",
            )
...
async def _report_incident(ctx: dict, exc_class: str, where: str, project_id: str = "cardloop") -> None:
    ...
    try:
        ...
        _proj_sk = proj.get("session_key", "")
        await _ingest_errors_to_board(proj["cwd"], proj["name"], [err], session_key=_proj_sk)
    except Exception:
        pass
...
async def _scan_and_ingest(project: dict, ctx: dict | None = None) -> dict:
    ...
    # Cockpit notification for NEW incidents (not dedup-updates)
    if added > 0 and ctx and project.get("notify_on_error"):
        try:
            await _notify_operator(ctx, f"[ERROR] {added} new incidents in {project['name']} — check the board.")
        except Exception as e:
            print(f"[scan_and_ingest] notify failed for {project['name']}: {e}")
...
def _scan_state_init(ctx: dict) -> None:
    ...
    try:
        ...
        pass
    # Prune orphan queue entries (spec-052 Phase 1b): a card id that no longer
    # exists in its project's TASKS.md (dismissed/archived after enqueue) would
    # otherwise make the drain loop fail forever on a phantom card. Conservative:
    # drop ONLY when we can positively confirm the id is absent from an EXISTING
    # board file; unknown cwd / missing TASKS.md → keep the entry.
    pruned = False
    topics = ctx.get("topics") or {}
    for k in list(_QUEUE.keys()):
        topic = topics.get(k)
        cwd = topic.get("cwd") if isinstance(topic, dict) else None
        if not cwd:
            continue
        ...
        if not tasks_path.exists():
            continue
        try:
            board_text = tasks_path.read_text(encoding="utf-8")
        except Exception:
            continue
        kept = [c for c in _QUEUE[k] if c in board_text]
        if len(kept) != len(_QUEUE[k]):
            pruned = True
            if kept:
                _QUEUE[k] = kept
            else:
...
#   always — auto-resume silently when the window resets
#   never  — do nothing
_AUTO_RESUME_MODES = ("ask", "always", "never")
...
# ─────────────────────── API: autopilot (spec-067 / spec-068) ───────────────────────
#
# All autopilot endpoints, the shadow loop, and the Director have been extracted
# into features/autopilot/ (spec-068 feature-module pattern).
# They are registered via features.autopilot.register(app, ctx) in startup()
# below — INSIDE the function body (deferred import, never at module top-level).
# See: features/autopilot/routes.py, features/autopilot/loop.py,
#      features/autopilot/director.py, features/autopilot/logic.py.
...
async def _notify_new_incidents(ctx: dict, project: dict, n_added: int) -> None:
    ...
    msg = f"[ERROR] {project['name']}: {n_added} new incidents\n{head}{more}\nBoard tab → Failed."
    await _notify_operator(ctx, msg)
...
async def _move_card_after_run(
    ...
    spec-052 Phase 2: emits board_event(moved) after a successful move."""
    try:
        target_col = "review" if ok else "failed"
        col_from: str | None = None
        async with _get_board_lock(cwd):
            _, preamble, cols = _load_board(cwd)
            # Determine source column before popping (for board_event column_from)
            for _ck, _cc in cols.items():
                if any(c["id"] == card_id for c in _cc):
                    col_from = _ck
                    break
        ...
        _emit_board_event(
            ...
            column_to=target_col,
            severity="info",
            summary=f"Card moved to {target_col} after run",
        )
        # spec-052 Phase 5: refresh the card's spec mirror (if linked).
        _sync_specs(session_key, cwd, [moved.get("spec")])
    except Exception as e:
        print(f"[_run_card] error moving card {card_id}: {e}")


# spec-039 removed the old TG-coupled rotation helpers (_notify_tg_rotation,
# _notify_tg_context_warn, _build_rotation_tail_prompt, _do_session_rotation).
# Auto-rotation is REVIVED (cost-tail control) but web-only and handoff-based now:
# the trigger is _maybe_auto_rotate (post-turn) reusing _rotate_session_core +
# the spec-042 handoff producer. CONTEXT_ROTATE_AT / CONTEXT_ROTATION / CONTEXT_WARN_AT
# are live again (see their definitions near the top of this file).
...
async def _run_card(
    ...
    try:
        ...
        await _move_card_after_run(ctx, cwd, name, card, card_id, ok, session_key=session_key)

        # Card-completion notification (cockpit activity bus)
        _ct_icon = "[OK]" if ok else "[ERROR]"
        _ct_short = (prompt[:60] + "…") if len(prompt) > 60 else prompt
        await _notify_operator(ctx, f"{_ct_icon} Card '{_ct_short}' → {'Review' if ok else 'Failed'}")
        ...
        # two chat strips at most (▶ started + outcome move), not three.
...
async def api_move_task(req: web.Request) -> web.Response:
    ...
    session_key = (project.get("session_key") or project.get("tg_thread", ""))
    _move_col_from: str | None = None
    async with _get_board_lock(cwd):
        _, preamble, cols = _load_board(cwd)
        # Find source column for board_event before popping (spec-052 Phase 2)
        for _ck, _cc in cols.items():
            if any(c["id"] == card_id for c in _cc):
                _move_col_from = _ck
                break
        ...
        if to == "done":
            ...
            # Shared archive helper keeps the ops:id marker so the spec mirror
            # (spec-052 P5/6) can still detect this card as done after it leaves
            # the open board. Same format as the reconcile archive site.
            new = header.rstrip() + "\n" + _done_archive_line(card, stamp)
    ...
    _queue_remove(session_key, card_id)
    # spec-052 Phase 2: emit moved event for manual moves (outside the lock, after board write)
    _move_to = to if to in cols else "backlog"
    _emit_board_event(
        session_key,
        event="moved",
        card_id=card_id,
        title=card.get("text", ""),
        column_from=_move_col_from,
        column_to=_move_to,
        severity="success" if _move_to == "done" else "info",
        summary="closed" if _move_to == "done" else f"moved to {_move_to}",
    )
    # spec-052 Phase 5/6: refresh the moved card's spec mirror (+ auto-close announce).
    _sync_specs(session_key, cwd, [card.get("spec")])
    return web.json_response(_board_payload_with_specs(cwd, ctx["DATA"]))
...
async def api_update_task(req: web.Request) -> web.Response:
    ...
    if update_spec:
        from board import _SPEC_ID_RE
        raw_spec = (body.get("spec") or "").strip().lower()
        if raw_spec and not _SPEC_ID_RE.fullmatch(raw_spec):
            return web.json_response(
                {"error": "spec: must be a slug like '052' (a-z0-9-, ≤24) or empty to clear"},
                status=400,
            )
        card_spec = raw_spec or None
    cwd, name = project["cwd"], project["name"]
    _touched_specs: set = set()
    async with _get_board_lock(cwd):
        _, preamble, cols = _load_board(cwd)
        found = False
        ...
        if not found:
            return web.json_response({"error": "card not found"}, status=404)
        _save_board(cwd, name, preamble, cols)
    # spec-052 Phase 5: refresh mirrors for the (old and/or new) linked spec.
    session_key = (project.get("session_key") or project.get("tg_thread", ""))
    _sync_specs(session_key, cwd, _touched_specs)
    return web.json_response(_board_payload_with_specs(cwd, ctx["DATA"]))
...
# Turns the per-turn rows engine.append_usage_ledger() writes into the attribution the audit asked
# for: cost/tokens by entrypoint (chat vs card vs deferred), the ultracode fan-out share, the heavy
# "tail" sessions (max context reached), and cost-per-day. Read-only; whole-file scan is fine at
# ~150 bytes/turn. Note cost_usd is list-price NOTIONAL (subscription is flat) — a relative signal.
def _read_usage_ledger(path: Path, since_ts: float) -> list[dict]:
    rows: list[dict] = []
    if not path.exists():
        return rows
    try:
        ...
        print(f"[ledger] read failed: {exc!r}")
    return rows


async def api_usage_ledger(req: web.Request) -> web.Response:
    ctx = req.app["ctx"]
    try:
        days = float(req.query.get("days", "7"))
    except (TypeError, ValueError):
        days = 7.0
    since_ts = time.time() - days * 86400.0
    path = ctx["DATA"] / "usage_ledger.jsonl"
    rows = _read_usage_ledger(path, since_ts)
    ...
    for r in rows:
        ...
        _add(by_entrypoint.setdefault(ep, _blank()), r)
        day = time.strftime("%Y-%m-%d", time.gmtime(r.get("ts") or 0))
        _add(by_day.setdefault(day, _blank()), r)
        if r.get("ultracode"):
            _add(ultracode, r)
        sk = str(r.get("session_key") or "?")
        s = sessions_agg.setdefault(sk, {**_blank(), "max_context_tokens": 0,
    ...
    top_sessions = sorted(
        ({"session_key": k, **v} for k, v in sessions_agg.items()),
        key=lambda x: x["max_context_tokens"], reverse=True,
    )[:15]
...
# Surfaces the latest model display names from the subscription's /v1/models listing so the UI
# can show current versions (e.g. "Opus 4.8") without hardcoding them. Best-effort: any failure
# (no token / network / non-200) falls back to the static labels below. Read-only and free.
...
async def _fetch_live_models() -> "list[dict] | None":
    """GET the subscription's /v1/models listing. Returns [{id, display_name}, ...] (newest first)
    or None on ANY failure. Never raises. The OAuth token is never logged."""
    token = _read_oauth_token()
    if not token:
        return None
    headers = {
    ...
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get(_MODELS_URL, headers=headers,
    ...
    for m in data:
        if isinstance(m, dict) and m.get("id"):
            out.append({"id": m["id"], "display_name": m.get("display_name") or m["id"]})
    return out


def _build_model_registry(live: "list[dict] | None") -> dict:
    """Combine the alias families with the live listing. For each family, pick the first live
    model whose id startswith 'claude-<family>' (API returns newest first → latest version) and
    strip a leading 'Claude ' from its display_name; otherwise use the static fallback label."""
    models: list[dict] = []
    for alias, static_label in _MODEL_FAMILIES:
        label = static_label
        if live:
            prefix = f"claude-{alias}"
            for m in live:
                if str(m.get("id", "")).startswith(prefix):
                    dn = str(m.get("display_name") or "").strip()
                    if dn.startswith("Claude "):
                        dn = dn[len("Claude "):].strip()
                    if dn:
                        label = dn
                    break
        models.append({"value": alias, "label": label})
...
async def api_models(req: web.Request) -> web.Response:
    ...
    if cached is None or (now - _models_cache["ts"]) > _MODELS_TTL:
        live = await _fetch_live_models()
        cached = _build_model_registry(live)
...
def _resume_mode_for(project: dict) -> str:
    """spec-051: effective rate-limit resume policy for a project/topic binding.

    Explicit per-project `auto_resume_mode` wins; otherwise fall back to the
    global env default (AUTO_RESUME_ON_RATE_LIMIT=1 → 'always', 0 → 'ask')."""
    mode = (project.get("auto_resume_mode") or "").strip().lower()
    if mode in _AUTO_RESUME_MODES:
        return mode
    return "always" if _AUTO_RESUME_ON_RATE_LIMIT else "ask"
...
async def _maybe_auto_resume(
    ...
    """Phase D + spec-051: resume after a rate-limit, per project policy.

    Called after any run completes (run_agent, api_project_chat, _run_card,
    _execute_deferred). If the run ended with api_error_status=429, consults the
    project's auto_resume_mode (_resume_mode_for):
    - always → create a fire_on_reset deferred record (status "pending"), silent.
    - ask    → create the record as "awaiting_confirmation" and publish a
               rate_limit_prompt event to the project chat (Yes/No + remember).
               api_deferred_confirm flips it to "pending" on Yes.
    - never  → do nothing.

    Chain guard: if parent_auto_resume_count >= AUTO_RESUME_MAX, notifies the
    operator instead of creating another auto-resume record.
    ...
    record = {
        ...
        "fire_on_reset": True,
        "created": _utcnow_iso(),
        # ask → wait for operator confirmation (loop ignores non-"pending");
        # always → queue immediately (silent, prior behaviour).
        "status": "pending" if mode == "always" else "awaiting_confirmation",
        "fired_at": None,
        "error": None,
        "attempts": 0,
        # Phase D fields
    ...
    if mode == "always":
        ...
        try:
            _bus_publish(session_key, {
                ...
                "project": project_name,
                "session_key": session_key,
                "resets_at_display": resets_at_str,
                "original_prompt_preview": short_original,
            })
        except Exception as e:
            print(f"[auto_resume] prompt publish error: {e}")
        await _notify_operator(
            ctx,
            f"⏸ {project_name}: rate-limited — continue when it resets?"
            + (f" (resets ~{resets_at_str})" if resets_at_str else "")
        )


def _get_resets_at_display(ctx: dict) -> str:
    """Returns a human-readable local-time string (OPERATOR_TZ, default UTC) for the next rate-limit reset, or ''."""
    try:
        from zoneinfo import ZoneInfo
        tz = ZoneInfo(os.environ.get("OPERATOR_TZ", "UTC"))
...
async def _notify_operator(ctx: dict, text: str) -> None:
    ...
    Non-critical — errors are logged, never raised."""
                            ...
                            if (not strict_reset) and util is not None and util < _DEFERRED_FREE_THRESHOLD:
                                ...
                                if resets_at is None:
                                    ...
                                    if jitter is None:
                                        ...
                                        changed = True
                                    if strict_reset:
                                        # Strict records: pin target on first acquisition.
                                        # After the 5h window resets, the API returns a NEW
                                        # resets_at for the fresh window (~5h in future), so
                                        # the live value is a moving target that never passes.
                                        # We compare against the ORIGINAL boundary instead.
                                        target = record.get("_target_resets_at")
                                        if target is None:
                                            target = resets_at
                                            record["_target_resets_at"] = target
                                            changed = True
                                        fire_now = (time.time() >= target + jitter)
                                    else:
                                        # Non-strict (auto-resume): use the live resets_at.
                                        # These records also fire via the free-window path
                                        # (utilization < threshold) once the window resets,
                                        # so the moving-target problem doesn't affect them.
...
def _set_project_resume_mode(ctx: dict, session_key: str, mode: str) -> int:
    """spec-051: persist auto_resume_mode to every topic sharing session_key's cwd.
    'ask' → reset (remove the key). Returns the number of topics updated."""
    topics = ctx.get("topics") or {}
    topic = topics.get(session_key)
    if topic is None:
        return 0
    cwd = topic.get("cwd")
    val = mode if mode in ("always", "never") else None
    changed = 0
    for b in topics.values():
        if b.get("cwd") == cwd:
            if val is None:
                b.pop("auto_resume_mode", None)
            else:
                b["auto_resume_mode"] = val
            changed += 1
    save_topics = ctx.get("save_topics")
    if callable(save_topics) and changed:
        save_topics()
    return changed


async def api_deferred_confirm(req: web.Request) -> web.Response:
    """POST /api/deferred/{id}/confirm — spec-051: resolve an awaiting_confirmation
    auto-resume record. Body: {decision: "yes"|"no", remember: bool}.

    yes → record becomes "pending" (the loop fires it on reset).
    no  → record becomes "cancelled".
    remember → write project policy: yes→always, no→never. Idempotent."""
    ctx = req.app["ctx"]
    deferred_id = req.match_info["id"]
    try:
        body = await req.json()
    ...
    if decision not in ("yes", "no"):
        return web.json_response({"error": "decision must be 'yes' or 'no'"}, status=400)
    ...
    if status != "awaiting_confirmation":
        # Already resolved (fired/cancelled/pending) → idempotent no-op.
        return web.json_response({"ok": True, "status": status, "noop": True})

    rec["status"] = "pending" if decision == "yes" else "cancelled"
    _save_deferred(records)

    policy = None
    if remember:
        policy = "always" if decision == "yes" else "never"
        _set_project_resume_mode(ctx, rec.get("session_key", ""), policy)

    return web.json_response({"ok": True, "status": rec["status"], "remembered": policy})
...
async def api_project_upload(req: web.Request) -> web.Response:
    ...
    # `path` is the absolute filesystem path the agent reads via the Read tool.
    # `url` is a servable HTTP path so the cockpit can preview images/videos inline
    # in the user's message bubble (spec-038-style ChatImage + Lightbox).
    url = f"/api/projects/{project['id']}/upload/{dest.name}"
    return web.json_response({"path": str(dest), "url": url, "name": filename, "size": size})


async def api_project_upload_file(req: web.Request) -> web.Response:
    """GET /api/projects/{id}/upload/{filename} — serve a user-uploaded inbox file.

    Mirrors api_project_media's guards, but the inbox is shared (DATA/inbox), so the
    {id} only gates auth. Only image/video types are served (inline preview); other
    types return 415 — the agent still reads them by absolute path, no HTTP needed.
    """
    ctx = req.app["ctx"]
    project = _find_project_by_id(ctx, req.match_info["id"])
    if project is None:
    ...
    try:
        ...
        real_inbox = os.path.realpath(str(inbox))
    except Exception:
        return web.json_response({"error": "invalid path"}, status=400)
    if not real_target.startswith(real_inbox + os.sep):
        return web.json_response({"error": "invalid filename"}, status=400)
...
async def api_project_chat(req: web.Request) -> web.Response:
    ...
    # Final context size this turn (for post-turn auto-rotate — captured from the result frame).
    ...
    try:
        ...
        # Cost control: if this turn pushed context past the cap, auto-rotate WITH a handoff so the
        ...
        if _chat_run_ok and _chat_final_ctx_tokens is not None:
            try:
                ...
                print(f"[auto-rotate] post-turn trigger error: {_ar_exc!r}")
...
async def _rotate_session_core(ctx: dict, project: dict, session_key: str, do_handoff: bool,
    ...
    `trigger` ("manual"|"auto") and `context_tokens` are recorded on the durable rotation event
    emitted to the project timeline on success, so the Activity feed answers "when & why".

    Atomically clears BOTH session layers (sessions.json derived cache + chats.json source-of-truth
    ...
    Returns:
      ...
      {"ok": True, "reset": False, "reason": str}    — nothing to clear
      {"busy": True}                                  — a turn is in flight; caller decides 409/skip
    """
    # Refuse if project is currently running (a turn in flight) — caller maps to 409 / skip.
    if ctx["running"].get(session_key) is not None:
        return {"busy": True}
    ...
    try:
        ...
        try:
            ...
            print(f"[rotate] live-client eviction error for {session_key}: {_exc!r}")

        # Durable rotation record → the project timeline (Activity feed). One emit point
        # covers BOTH manual (/rotate) and auto (_maybe_auto_rotate) rotations; `trigger`
        # + context_tokens answer "when & why". persist=True writes the timeline JSONL and
        # fans out live so an open Activity tab updates instantly. Best-effort.
        try:
            _bus_publish(session_key, {
        ...
        except Exception as _rot_evt_exc:
            print(f"[rotate] timeline emit failed (non-blocking): {_rot_evt_exc!r}")
...
async def _maybe_auto_rotate(ctx: dict, project: dict, session_key: str, ctx_tokens: int,
    ...
    try:
        ...
        if result.get("busy"):
            print(f"[auto-rotate] skipped {session_key}: a turn is already in flight")
            return
        if result.get("reset"):
            print(f"[auto-rotate] rotated {session_key} at {ctx_tokens} tok "
                  f"(>= {CONTEXT_ROTATE_AT}), handoff={result.get('handoff')}")
            # Live toast only — the durable timeline record is emitted by _rotate_session_core
            # as kind:"session_rotated" (persist=False here avoids a duplicate timeline entry).
            _bus_publish(session_key, {
                "kind": "auto_rotated",
                "source": "chat",
                "context_tokens": ctx_tokens,
                "threshold": CONTEXT_ROTATE_AT,
                "handoff": bool(result.get("handoff")),
            }, persist=False)
    except Exception as exc:
        print(f"[auto-rotate] failed for {session_key} (non-blocking): {exc!r}")
...
async def _build_handoff_inner(ctx: dict, session_key: str, cwd: str, session_id: "str | None") -> str:
    ...
    try:
        ...
        if full_dialog:
            ...
            if len(full_dialog) <= _HANDOFF_CHUNK_CHARS:
                ...
                # spec-056: when the dialog exceeds the recency tail threshold, split into
                # EARLIER (compressed to one paragraph) and RECENT TAIL (analyzed in detail).
                if len(full_dialog) > _HANDOFF_TAIL_CHARS:
                    earlier_zone = full_dialog[:-_HANDOFF_TAIL_CHARS]
                ...
                for i, chunk in enumerate(chunks):
                    ...
                    if is_last:
                        # spec-056: last chunk — larger budget, preserve detail
                        chunk_prompt = (
                            f"Summarize this portion (part {i + 1}/{len(chunks)}, "
                            "most recent — preserve detail) of the session for handoff. "
                            "Key decisions, actions, file changes, open questions, current state. "
                            "Dense, ≤350 words, English.\n\n"
                            + chunk
                        )
                    else:
                        chunk_prompt = (
    ...
    # spec-056: deterministic tail — verbatim last user instruction and last agent message.
    # These survive even if the narrative model misses the stop point.
    try:
        last_user_text = ""
        last_assistant_text = ""
        for entry in history:
...
async def api_terminal_ws(req: web.Request) -> web.WebSocketResponse:
    ...
    # Set default terminal size to 80x24 to avoid 0x0 grid on early output
    try:
    ...
    # Resolve D-Bus user socket dynamically (critical for keyring auth in systemd services)
    try:
        uid = os.getuid()
    ...
    loop = asyncio.get_running_loop()
    # Bounded queue caps memory if the frontend lags. On overflow we apply
    # BACKPRESSURE (pause reading the PTY) instead of dropping bytes: dropping a
    # 4 KiB chunk would truncate an escape sequence mid-stream and desync the xterm
    # parser (garbled or stuck screen). Pausing lets the kernel PTY buffer fill, which
    # naturally blocks the producing program — exactly how a real terminal throttles.
    ...
    def _on_pty_readable() -> None:
        try:
            ...
            # PTY closed (child exited): stop reading and signal EOF without losing it.
            _pause_reader()
            try:
                pty_queue.put_nowait(None)
            except asyncio.QueueFull:
                loop.create_task(pty_queue.put(None))
            return
        if not data:
            return
        # Always fits: we pause the reader the instant the queue fills, so a free slot
        # is guaranteed for the chunk we just read (the event loop is single-threaded).
    ...
    async def _flush_to_ws() -> None:
        while True:
            ...
            try:
                ...
                return
            # A slot just freed — resume reading the PTY if backpressure had paused it.
            _resume_reader()
...
async def api_totp_enroll(req: web.Request) -> web.Response:
    ...
    ctx = req.app["ctx"]
    issuer = os.environ.get("TOTP_ISSUER", "Cardloop")
...
async def api_new_project(req: web.Request) -> web.Response:
        ...
        try:
            _git_env = {
                **os.environ,
                ...
                "GIT_AUTHOR_EMAIL": "cardloop@localhost",
                "GIT_COMMITTER_NAME": "Cardloop",
                "GIT_COMMITTER_EMAIL": "cardloop@localhost",
            }
    ...
    # spec-040: new projects get a slug-based session key (transport-neutral).
...
def _detect_error_handler(cwd: Path, claude_md_text: str) -> bool:
    ...
    try:
        ...
        if _conformance_heading:
            section = claude_md_text.split(_conformance_heading, 1)[1].split("\n## ", 1)[0]
...
- `TASKS.md.tpl` — card format preamble. If the current TASKS.md lacks a preamble with the phrase "Card format" — add it BEFORE the first `##` column.
...
async def start(ctx: dict) -> None:
    ...
    try:
        ...
        # spec-059 Move 1: epic-lens — specs (docs/internal/specs) as epics w/ card progress
        app.router.add_get("/api/projects/{id}/epic-specs", api_project_epic_specs)
        app.router.add_get("/api/projects/{id}/epic-specs/{name}", api_project_epic_spec_content)
        ...
        # Serve user-uploaded files back for inline preview in the chat (auth-guarded)
        app.router.add_get("/api/projects/{id}/upload/{filename}", api_project_upload_file)
        ...
        app.router.add_post("/api/deferred/{id}/confirm", api_deferred_confirm)
