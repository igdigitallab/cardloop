...
- [x] 99% работу у меня сейчас идет через наш сервис, вот этот, но иногда нужно сделать какие-то выполнить какие-то команды без участия чата или же клода. То есть сделать это просто через терминал. я хочу добавить новый функционал сделать некую может быть добавить просто чистый терминал. Как новая вкладка, чтобы открывался просто чистый терминал. · 2026-06-22
- [x] Feature: Make global settings easily accessible <!--ops:921e4a-->
- [x] Иногда в чате клод мне пишет, что нужно скопировать, допустим, и вставить там куда-либо. И вот некий текст он пишет там в рамочку- это круто, красиво, мне нравится, но было бы замечательно, если бы там была кнопочка Знаете, знаешь, как скопировать, чтобы сохранилось в буфер обмен. И еще после сообщения к лода вообще, то есть любого законченного ответа внизу во всех современных чатах. Чатах, можно тоже копировать там весь ответ.Не выделяя его руками. · 2026-06-22
- [x] Нам в чат можно подгрузить картинку причём там подгружается путь до картинки я хочу сделать так же чтобы можно было подгружать картинки в бэк Лок для задач ну там не только картинки файл картинки и прочее то есть <!--ops:74aaac-->
- [x] Ещё беклог нет привязки по ширине смартфона телефон через мобильную версию в общем отображается плохо все карточки уезжают вправо <!--ops:7f6ff6-->
- [x] Deploy Cardloop на VM племяннице <!--ops:824bcd-->
- [x] Redesign mobile tab bar: nativ nav-bar + master-detail (Option A) <!--ops:eee093-->
