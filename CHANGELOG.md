...
### Added
- **Session goal 🎯 (spec-076)** — pin a goal on any chat and the agent literally cannot stop until it's done: the backend registers the CLI's NATIVE `/goal` machinery (a prompt-type Stop hook via `--settings`), so every stop attempt is judged by an LLM evaluator against the transcript — unmet → the turn is blocked with feedback and the agent keeps working (consecutive-block cap 8, env `CLAUDE_CODE_STOP_HOOK_BLOCK_CAP`). One slim pinned bar above the chat (condition + live "attempt N" status, inline edit/clear, mobile-friendly), set via the model pill menu's 🎯 row; the goal lives on the chat object in `chats.json`, is enforced on queued/auto-continue turns too (drain-path parity), auto-clears on "achieved" (green ✓) and composes with ultracode in one settings payload. The engine emits `goal_status` events (blocked attempts + terminal verdict) that stream to the UI and persist a `goal_met` timeline row.
